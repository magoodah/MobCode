...
/**
 * Hook for assembling status messages for this shared object.
 * Return null to skip sending a message.
 * @return {Object} Object filled with key/value message pairs.
 */
mobwrite.shareObj.prototype.getMessages = function() {
  return null;
...
/**
 * Hook for parsing the status messages for this shared object from other
 * collaborators.
 * @param {Object?} messages Object filled with sub-objects filled with
 * key/value message pairs.  Each sub-object is from one collaborator.
 * Null is passed if no message was received from the server.
 */
mobwrite.shareObj.prototype.setMessages = function(messages) {
  //for (var who in (messages || {})) {
  //  alert('Message from "' + who + '":\n' + messages[who]);
  //}
};
...
mobwrite.shareObj.prototype.syncText = function() {
  ...
  // Create the message string for this file.
  if ('JSON' in window) {
    var messages = this.getMessages();
    if (messages !== null) {
      data += 'M:' + window.JSON.stringify(messages) + '\n';
    }
  }
...
/**
 * Encode protocol data into JSON-P URLs.  Split into multiple URLs if needed.
...
mobwrite.syncRun2_ = function(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (command == 'F' || command == 'f') {
      if (file && !gotMessage) {
        // No messages received on previous file.  Fire empty message event.
      ...
      if (file && 'JSON' in window) {
        var messages = null;
        try {
          messages = window.JSON.parse(value);
        } catch (e) {
          // Given that the server should have parsed this message, this should
          // never happen.
          if (mobwrite.debug) {
            window.console.severe('Message parse error.\n' + encodeURI(value));
          }
        }
...
mobwrite.computeSyncInterval_ = function() {
  ...
  if (mobwrite.clientChange_ || mobwrite.serverChange_) {
    // Activity.  Cut the sync interval to zero.
    mobwrite.syncInterval = 0;
  } else {
    // No activity.
...
/**
 * Hook for user-notification that a new sync has started.
 */
...
/**
 * Hook for user-notification that the previous sync finished successfully.
 */
...
/**
 * Hook for user-notification that the previous sync failed and will retry.
 */
